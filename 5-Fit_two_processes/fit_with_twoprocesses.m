...
%_________________________FIT WITH TWO PROCESSES___________________________
%
...
%Lower bounds
LB=[1e-10 1e-15 0 1e-10 1e-15 0 zeros(1,num_bineyes)];

%Upper bounds
UB=[10 1 5 10 1 5 ones(1,num_bineyes)];
...
tic,
clear garesult
[garesult.x,garesult.fval,garesult.exitflag,garesult.output,garesult.population,garesult.scores] = ga(@(var)functomin_twoprocesses(rf,rferr,inf,inferr,funcorr,funcorrerr,maxfiberlength,var,interval),nvars,[],[],[],[],LB,UB,[],options);
toc
if garesult.x(1)>=garesult.x(4)
    results(i,:)=[garesult.fval,garesult.x];
else
    results(i,:)=[garesult.fval,garesult.x(4:6),garesult.x(1:3),1-garesult.x(7:end)];   
end
